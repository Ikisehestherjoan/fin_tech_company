...
ft_customer_transactions = '''
    ...
    FROM dev.transactions t
    LEFT JOIN dev.wallets w
        ON t.wallet_id = w.id
    LEFT JOIN dev.cards c
        ON w.associated_card = c.id
    LEFT JOIN dev.transaction_status ts
...
dim_wallets = '''
    ...
    FROM dev.wallets w
    LEFT JOIN dev.users u
        ON w.owner_id = u.id
    LEFT JOIN dev.cards c
        ON w.associated_card = c.id
    LEFT JOIN dev.banks b
        ON b.id = c.issuer;
...
dim_customer = '''
    ...
    FROM dev.users u
    LEFT JOIN dev.cust_verification_status cvs
        ON u.id = cvs.user_id;
'''
